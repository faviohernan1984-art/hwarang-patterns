...
import {
  ...
  roomSubmissionsColRef, roomSubmissionsQuery, roomSubmissionRef, roomPublicStateRef,
} from "./firebase.js";
...
test("Firebase references are scoped to exactly one room", () => {
  ...
  assert.equal(roomPublicStateRef("A").path, "rooms/A/publicState/current");
  ...
  assert.equal(roomPublicStateRef("B").path, "rooms/B/publicState/current");
